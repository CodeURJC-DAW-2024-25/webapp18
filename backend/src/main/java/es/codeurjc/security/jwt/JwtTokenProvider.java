...
public class JwtTokenProvider {
	...
	public String getUsername(String token) {
		return ((JwtParserBuilder) Jwts.builder()).setSigningKey(Keys.hmacShaKeyFor(jwtSecret.getBytes())).build().parseClaimsJws(token).getBody().getSubject();
	}
	...
	public boolean validateToken(String token) {
		try {
			((JwtParserBuilder) Jwts.builder()).setSigningKey(jwtSecret.getBytes()).build().parseClaimsJws(token);
			return true;
